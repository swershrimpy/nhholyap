"""
animate_refinement_3d_chase_twopanel.py
========================================
Two-panel matplotlib animation of the 3-D refinement interval plot with chase view.
...
curve), the same chase camera that follows the trajectory from behind,
and the same static axis limits.
...
--------
  ...
  cam_dist       = 2.0      (camera distance from subject)
  cam_offset     = 0.0      (azimuth offset in degrees, 0 = directly behind)

Usage
-----
  python animate_refinement_3d_chase_twopanel.py --fault-mode actuator
  python animate_refinement_3d_chase_twopanel.py --fault-mode actuator --x0 0.2 0.0 0.5
"""
...
DEFAULT_CAM_DIST = 2.0
DEFAULT_CAM_OFFSET = 0.0
PAIR_I, PAIR_J  = 0, 1
...
def parse_args():
    ...
    p.add_argument(
        "--cam-dist", type=float, default=DEFAULT_CAM_DIST,
        help=f"Camera distance from subject (default: {DEFAULT_CAM_DIST}).",
    )
    p.add_argument(
        "--cam-offset", type=float, default=DEFAULT_CAM_OFFSET,
        help=f"Camera azimuth offset in degrees (default: {DEFAULT_CAM_OFFSET}, 0 = directly behind).",
    )
...
def compute_chase_view_params(traj_obs_px, traj_obs_py, traj_times, frame_idx,
                              cam_dist, cam_offset_deg):
    """Compute camera position for chase view at given frame.
    
    The camera follows the subject from directly behind, maintaining a fixed
    distance and looking toward the subject.
    
    Returns (azim, elev) for view_init.
    """
    # Current subject position (trajectory point at this frame)
    px = float(traj_obs_px[frame_idx])
    py = float(traj_obs_py[frame_idx])
    t = float(traj_times[frame_idx])
    
    # Look ahead to next point to determine direction
    if frame_idx + 1 < len(traj_obs_px):
        px_next = float(traj_obs_px[frame_idx + 1])
        py_next = float(traj_obs_py[frame_idx + 1])
        dx = px_next - px
        dy = py_next - py
    else:
        # Use previous point if at end
        if frame_idx > 0:
            px_prev = float(traj_obs_px[frame_idx - 1])
            py_prev = float(traj_obs_py[frame_idx - 1])
            dx = px - px_prev
            dy = py - py_prev
        else:
            dx, dy = 1.0, 0.0  # Default forward direction
    
    # Calculate direction angle (azimuth in xy plane)
    direction_angle = np.arctan2(dy, dx) * 180 / np.pi
    
    # Camera is directly behind the subject (0 deg offset = directly behind)
    # Directly behind = direction + 180 degrees
    camera_azim = direction_angle + 180 + cam_offset_deg
    
    # Elevation: look slightly down at the subject from above
    camera_elev = 15.0
    
    return camera_azim, camera_elev
...
def draw_unrefined_panel(ax, ctrl_k, traj_end_idx,
                         ...
                         total_time, cam_dist, cam_offset_deg):
    ...
    # Chase view: camera follows the subject
    azim, elev = compute_chase_view_params(
        traj_obs_px, traj_obs_py, traj_times, traj_end_idx, cam_dist, cam_offset_deg)
...
def main():
    ...
    cam_offset_deg = args.cam_offset
    ...
    print(f"Camera     : dist={cam_dist} offset={cam_offset_deg}deg (chase view)")
    ...
    out_path = args.output or str(
        HERE / f"refinement_anim_chase_twopanel_{fault_mode}_ctrl{ctrl_dt}x{ctrl_num_steps}.mp4"
    )
