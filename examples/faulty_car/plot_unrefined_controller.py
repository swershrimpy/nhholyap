...
Four figures are produced (one PDF each):
...
used – the propagation is simply using the optimal multi-step control obtained
by calling ``optimize_multistep_gpu`` from ``faulty_car_separating_input.py``.
"""
...
dt = 0.1
steps_per_segment = 1   # making num_steps correspond to number of segments
num_segments = 50
...
# util to draw overlap boxes given list of intervals at time t
...
# figures for individual pairs (also used in the combined layout below)
pair_titles = ['Nominal+Actuator','Nominal+Sensor','Actuator+Sensor']
def draw_pair_plot(ax, i, j, pair_title):
    """Draw on existing 3D axis the two scenarios i,j plus overlap boxes."""
    all_px, all_py = [], []
    for step_idx, t in zip(step_indices, times):
        ...
        if xh>=xl and yh>=yl:
            ...
            corners_y=[t,t,t,t,t]
            corners_z=[yl,yl,yh,yh,yl]
            ax.plot(corners_x,corners_y,corners_z,color='black',linewidth=2.0,zorder=5)
    pad=0.05
    ax.set_xlim(min(all_px)-pad,max(all_px)+pad)
    ax.set_ylim(times[0], times[-1])
    ax.set_zlim(min(all_py)-pad,max(all_py)+pad)
...
# individual pair files (optional, kept for compatibility)
for idx,(i,j) in enumerate(pairs, start=1):
    fig = plt.figure(figsize=(10,7))
    ax = fig.add_subplot(111, projection='3d')
    draw_pair_plot(ax, i, j, f'Refined pair: {pair_titles[idx-1]}')
    plt.tight_layout()
...
# panels 1–3: pair plots
for idx,(i,j) in enumerate(pairs, start=1):
    draw_pair_plot(axes[idx], i, j, pair_titles[idx-1])

# build shared legend
# create dummy intersection handle
