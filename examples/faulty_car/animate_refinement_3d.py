...
--------------------------------------
  ctrl_dt / ctrl_num_steps
      The controller's Euler step size and number of steps.  These define:
        ...
        - the total time axis length     (ctrl_dt × ctrl_num_steps = 5 s default)
  ...
  sim_dt  (--sim-dt, default 0.033 s ≈ 30 fps)
      ...
      time axis always spans exactly ctrl_dt × ctrl_num_steps seconds.
...
- px/py axes: fixed limits computed once from all data (trajectory + all interval
  boxes), padded by LIMIT_PAD.  Constant throughout the animation.
...
def main():
    ...
    # Each animation frame corresponds to a real time t = frame * sim_dt.
    # We map that to the nearest controller step index.
    num_anim_frames = max(1, round(total_time / sim_dt)) + 1
    anim_times      = np.linspace(0.0, total_time, num_anim_frames)
    # For each animation frame, find the nearest controller step index
