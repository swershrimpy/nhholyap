"""
Standalone intersection-refinement SCENARIO-COUNT runtime/RAM/VRAM sweep at
the wide-restart, 8-GD-iteration config. See
unrefined_scenario_scaling_wide_gd8.py's module docstring for why this is a
STANDALONE copy of refined_scenario_scaling.py's sweep logic rather than an
env-var override of that shared script -- another process editing that exact
file mid-run silently reverted this job's config partway through its first
attempt. Depending only on nonlinear_chain_separating_input.py and
runtime_scaling_common.py (both confirmed git-clean and unchanged by that
process) and owning this file exclusively removes that race entirely.

Same per-point subprocess isolation as refined_horizon_scaling.py's
zeroth_wide config: WORKER_TIMEOUT_S / WORKER_MEM_LIMIT_MB are math.inf (not
the original script's 150s / 4096MB guard rails, sized for the
20-restart/15-iter config).

See unrefined_scenario_scaling_wide_gd8.py for the unrefined counterpart.
"""
...
N = 10
CSV_PATH = _HERE / "refined_scenario_scaling_wide_gd8.csv"
RUN_TIME_REPEATS = 100

DT = 0.02
NUM_STEPS = 3
NUM_RESTARTS = 200_000
NUM_ITERS = 8
LEARNING_RATE = 0.1
...
def run_for_scenario_count(total_scenarios: int):
    print(f"\n{'=' * 70}\nREFINED MULTISTEP (wide, gd8) -- {total_scenarios} scenarios (N={N})\n{'=' * 70}")
