...
X_PERTURBATION = np.array([.1, .1, 0.1, .1])
mu = np.array([10, 10])
...
A_ldi_list = []
B_ldi_list = []
A_index_map = []  # Stores (i, start_idx, end_idx)
B_index_map = []  # Stores (i, start_idx, end_idx)

current_A_index = 0  # Keeps track of the next available index in all_A_corners
current_B_index = 0  # Keeps track of the next available index in all_B_corners
...
A_ldi_union = irx.Interval(lower=reduce(jnp.minimum, [A.lower for A in A_ldi_list]), upper=reduce(jnp.maximum, [A.upper for A in A_ldi_list]))
B_ldi_union = irx.Interval(lower=reduce(jnp.minimum, [B.lower for B in B_ldi_list]), upper=reduce(jnp.maximum, [B.upper for B in B_ldi_list]))
...
# Save A_list and B_list
with open("polytope_data.pkl", "wb") as f:
    pickle.dump({"A_list": A_list, "B_list": B_list, "A_ldi_list": A_ldi_list, "B_ldi_list": B_ldi_list}, f)
print(A_list)
