"""
Standalone multistep REFINED (intersection-refinement) separating-input
script for ADMIRE.

Solves for a control sequence u_seq (num_steps, 10) that separates all 11
fault scenarios via propagate_with_refinement_admire (at each step, every
...
-- see that function's docstring for the memory-optimized vmap-over-
scenarios/pairs implementation). Uses optimize_refined_sequence_gpu_fused
(this session's addition: fuses batched_loss/batched_grad into one
jax.vmap(jax.value_and_grad) call), wrapped in a single top-level jax.jit.

Compile-cost note: see multistep_unrefined_demo.py's module docstring --
a single unbatched gradient of the (simpler) unrefined loss already took
~266-271s on this machine, identical on GPU/CPU (compile-bound, not
GPU-acceleratable). The refined loss additionally vmaps over 55 scenario
pairs per step, so expect this script's compile to take AT LEAST as long,
plausibly several minutes more.

IMPORTANT correctness caveat (see faulty_car_output_feedback_cbf.py's
session history, examples/faulty_car/): a low/zero REFINED loss is a
genuine diagnosability certificate under the refinement mechanism's own
semantics, but is NOT automatically the same as the RAW (independent
per-scenario) reachable boxes being disjoint -- refined boxes are built by
intersecting scenarios' PREDICTED outputs, a proxy for what an online
observer would see, not the same object as independent propagation. This
script checks the raw per-segment disjointness directly (not just trusting
the loss value) before reporting a result as genuine, and prints both.

Usage:
  /home/user/immrax-venv/bin/python examples/admire/multistep_refined_demo.py
"""
import sys
import time
...
from admire_separating_input import create_scenarios, _propagate_history
from admire_refined_sequence_optimizer import optimize_refined_sequence_gpu_fused

# ── Configuration ────────────────────────────────────────────────────────────
DT = 0.1
NUM_STEPS = 10            # 1.0s horizon
NUM_RESTARTS = 50
NUM_ITERS = 40             # tuned short -- see module docstring: this is the
                           # controllable run-time knob, NOT the compile-time one
LEARNING_RATE = 0.05
SEED = 42
...
def main():
    ...
    def run(seed):
        return optimize_refined_sequence_gpu_fused(
    ...
    print(f"\nBest (refined) loss: {float(best_loss):.6f} rad^2  "
         f"({n_nan}/{NUM_RESTARTS} restarts NaN)")
    ...
    # ── Raw (independent, NOT refined) per-segment disjointness check --
    #    the real diagnosability question; see module docstring caveat.
    hist_by_name = {
