...
unicycle's solved separating-input controller, driven on REAL
...
# Plot the OBSERVED trajectory (== measured_xy for Nominal/Actuator Fault,
# but the sensor-transformed point for Sensor Fault -- see module docstring
# "Fault emulation on real hardware"), since that's what the tubes and the
# diagnosis check below actually compare against; plotting true state here
# for Sensor Fault would show a trajectory the diagnoser never sees.
ax.plot(observed_xy[:, 0], observed_xy[:, 1], "k.-", linewidth=2,
           markersize=8, zorder=10)
_traj_label = "Observed trajectory" if TRUE_MODE == "Sensor Fault" else "Measured trajectory"
legend_handles.append(mlines.Line2D([0], [0], color="k", marker=".", label=_traj_label))
ax.legend(handles=legend_handles, loc="upper left", fontsize=8)
