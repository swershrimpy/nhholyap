...
Reproduces examples/faulty_car/animate_refinement_3d_twopanel.py's output,
but calls this folder's car_separating_input.collect_refinement_history for
the per-pair refinement math instead of re-deriving it locally (see
animate_refinement_3d.py / PLAN.md bug #3: the original's local
reimplementation had silently drifted from the optimizer's own refinement
logic). The unrefined (left) panel here also goes through
car_separating_input.observed_output, so the Sensor Fault scenario's
obs_offset/obs_scale are applied consistently with the optimized loss
(PLAN.md bug #1) instead of the original's raw-state comparison.
...
def collect_unrefined_history(x0_ivl, u_seq, scenarios, dt, num_steps):
    """Propagate each scenario independently (no refinement), through this
    module's shared euler_step/observed_output helpers.
    Returns list of dicts: steps[k]["t"], steps[k]["obs"][sc_idx].
    """
    n  = len(scenarios)
    xs = [euler_step(s.emb_system, x0_ivl, u_seq[0], s.p_interval, dt)
          for s in scenarios]
    steps = [{"t": dt, "obs": [observed_output(xs[k], scenarios[k]) for k in range(n)]}]
    for step_k in range(num_steps):
        ...
        steps.append({"t": (step_k + 2) * dt,
                      "obs": [observed_output(xs[k], scenarios[k]) for k in range(n)]})
    return steps
    ...
    for pi in range(n_pairs):
        ...
        for k, step in enumerate(ref_steps):
            if not still_valid:
                valid[pi, k] = False
                continue
            obs_i, obs_j = step["pair_obs"][pi][0], step["pair_obs"][pi][1]
...
def compute_static_limits(unref_steps, ref_steps, ref_pairs, pair_idx,
    ...
    for step in ref_steps:
        obs_i, obs_j = step["pair_obs"][pair_idx][0], step["pair_obs"][pair_idx][1]
...
def draw_refined_panel(ax, ctrl_k, traj_end_idx,
    ...
    for s_idx in range(min(ctrl_k, len(ref_steps))):
        ...
        box_alpha = 0.15 if (inv_step is not None and s_idx >= inv_step) else 0.35
...
def draw_status_panel(ax_status, ctrl_k, scenarios, cmap, model_elim, ctrl_dt):
    ...
    Each model occupies one row:
      - Active     : coloured name, bold, with a checkmark prefix
      - Eliminated : grey name with strikethrough line + "@ t=Xs" annotation
    """
    ax_status.cla()
    ...
    for sc_idx, scenario in enumerate(scenarios):
        ...
        if eliminated:
            inv_t = int(round(elim_step * ctrl_dt))
        ...
        ax_status.axhline(row_y - 0.5, color="#eeeeee", linewidth=0.8)
...
def main():
    ...
    traj_obs_px = traj_fine[:, 0] * obs_scale + obs_off[0]
    traj_obs_py = traj_fine[:, 1] * obs_scale + obs_off[1]
