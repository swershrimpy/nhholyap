...
ONE unicycle config (multistep_unrefined/config_idx=67), run back-to-back in one Robotarium session.
...
Config selection: method=multistep_unrefined, config_idx=67 from
success_rate_data_early_stop_multistep_unrefined.npz -- chosen over the other 9
scripts in this family (main + _b.._j) for a WIDER x0_width AND a larger
worst-case tube margin in local-simulator dry runs, in response to real
Robotarium hardware showing a higher misdiagnosis rate than the (idealized,
noiseless) local simulator predicted: a wider assumed x0 uncertainty ball
and more slack between the actual trajectory and its tube boundary both
directly buy robustness against real-hardware effects (wheel slip, encoder
noise, imperfect reset convergence) the local simulator doesn't model.
Worst-case (over 3 repeated local-simulator dry runs, all 3 modes, all 10
segments) distance from the true/observed trajectory to its own tube's
nearest edge: +0.053 m -- better margin than the two already-deployed
x0_width=0.07 scripts in this family at the time this was chosen.
x0_width=0.07 (one step below this family's 0.08 ceiling -- see
robotarium_submit_diagnosis_all_modes_k.py's docstring for why 0.08 is the
practical ceiling for this project's config sweep).
u_opt satisfies real Robotarium hardware velocity limits (max|v|=0.1585 m/s,
max|omega|=0.4942 rad/s, vs. limits 0.2 m/s / 3.6364 rad/s).
...
# "Regenerating the embedded constants". method=multistep_unrefined, config_idx=67.
# ══════════════════════════════════════════════════════════════════════════
U_OPT = np.array([
    [0.07277888059616089, 0.4941975176334381],
    [0.07762090861797333, 0.49294301867485046],
    [0.08720782399177551, 0.4908463954925537],
    [0.10037069767713547, 0.4880281090736389],
    [0.1148362085223198, 0.4849574565887451],
    [0.12954680621623993, 0.4817081689834595],
    [0.143352672457695, 0.47867029905319214],
    [0.1538132131099701, 0.4769738018512726],
    [0.15847669541835785, 0.4765024185180664],
    [0.1579415202140808, 0.4775134325027466],
...
X0_WIDTH = 0.07000000029802322   # half-width car_separating_input.py's icentpert()
...
TUBE_LO = {
    "Nominal": np.array([
    [-0.24347636103630066, 0.3052215278148651],
    [-0.20958861708641052, 0.2865058481693268],
    [-0.16754041612148285, 0.2753819525241852],
    [-0.11793921887874603, 0.27468717098236084],
    [-0.06452511996030807, 0.287711501121521],
    [-0.011602322570979595, 0.3170388340950012],
    [0.035524651408195496, 0.3637147843837738],
    [0.07099869847297668, 0.4261305034160614],
    [0.08996403217315674, 0.4994758367538452],
    [0.09026191383600235, 0.5773985981941223],
    ]),
    "Actuator Fault": np.array([
    [-0.24540871381759644, 0.30319273471832275],
    [-0.21681253612041473, 0.2769710421562195],
    [-0.18227030336856842, 0.2503822147846222],
    [-0.1400417536497116, 0.22329367697238922],
    [-0.08926628530025482, 0.1965220868587494],
    [-0.029639238491654396, 0.17126873135566711],
    [0.03797691687941551, 0.14896149933338165],
    [0.1070542186498642, 0.13121293485164642],
    [0.17202576994895935, 0.11941288411617279],
    [0.2290426641702652, 0.11420255154371262],
    ]),
    "Sensor Fault": np.array([
    [-0.03130254149436951, 0.4899604320526123],
    [0.0008908212184906006, 0.4721805453300476],
    [0.040836602449417114, 0.46161288022994995],
    [0.08795774728059769, 0.4609528183937073],
    [0.13870114088058472, 0.4733259081840515],
    [0.18897779285907745, 0.5011869072914124],
    [0.2337484210729599, 0.5455290675163269],
    [0.2674487829208374, 0.6048239469528198],
    [0.2854658365249634, 0.6745020151138306],
    [0.28574880957603455, 0.7485286593437195],
    ]),
}
TUBE_HI = {
    "Nominal": np.array([
    [-0.10027813166379929, 0.44916513562202454],
    [-0.06411031633615494, 0.43536141514778137],
    [-0.02092125453054905, 0.4302138686180115],
    [0.02922087162733078, 0.43651077151298523],
    [0.08499818295240402, 0.45719099044799805],
    [0.14257673919200897, 0.49426597356796265],
    [0.19674700498580933, 0.5480444431304932],
    [0.24136726558208466, 0.616075873374939],
    [0.27085593342781067, 0.6928186416625977],
    [0.2821444869041443, 0.7716864347457886],
    ]),
    "Actuator Fault": np.array([
    [-0.10112608969211578, 0.44791722297668457],
    [-0.06696439534425735, 0.4295865297317505],
    [-0.025793863460421562, 0.4153645932674408],
    [0.023586122319102287, 0.4067089855670929],
    [0.08095354586839676, 0.4058261215686798],
    [0.1457270234823227, 0.41496530175209045],
    [0.21740347146987915, 0.43597471714019775],
    [0.2943100035190582, 0.46960702538490295],
    [0.3735484182834625, 0.51482754945755],
    [0.45251935720443726, 0.569352388381958],
    ]),
    "Sensor Fault": np.array([
    [0.10473577678203583, 0.6267068982124329],
    [0.13909520208835602, 0.613593339920044],
    [0.18012481927871704, 0.6087031960487366],
    [0.22775983810424805, 0.6146852374076843],
    [0.28074827790260315, 0.634331464767456],
    [0.33544790744781494, 0.6695526838302612],
    [0.3869096636772156, 0.7206422090530396],
    [0.42929890751838684, 0.7852720618247986],
    [0.4573131203651428, 0.858177661895752],
    [0.4680372476577759, 0.9331020712852478],
    ]),
}
