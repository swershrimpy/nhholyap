...
def print_constants(c: dict):
    ...
    # X0_WIDTH must be pasted in too, not just X0_CENTER -- the submission
    # scripts derive their RESET_POSITION_ERROR/RESET_ROTATION_ERROR reset
    # tolerance as a fraction of it (min(0.03, 0.4*X0_WIDTH)), not a fixed
    # constant, since a flat tolerance can be LOOSER than a tight config's own
    # assumed x0 uncertainty (config_idx=57 in this project's sweep has
    # X0_WIDTH=0.02) and cause the reset routine to consider itself "done" at
    # a residual pose error the tube computation never allowed for -- this
    # was caught as an intermittent MISSED diagnosis on real dry runs against
    # the local simulator (see robotarium_submit_diagnosis_all_modes.py's own
    # RESET_POSITION_ERROR comment for the full story).
    print(f"X0_WIDTH = {c['x0_width']!r}")
