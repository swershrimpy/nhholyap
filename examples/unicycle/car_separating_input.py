...
def propagate_with_refinement(x0_ivl: irx.Interval, u_seq: jnp.ndarray,
    ...
    def step_body(carry, u_k):
        ...
        new_carry = (jnp.stack(new_pxi), jnp.stack(new_pxj), jnp.minimum(min_cost, step_cost))
        return new_carry, None

    init_carry = (pxi_arr, pxj_arr, step1_cost)
    # scan over u_seq[1:num_steps] directly instead of a dynamic index k+1 --
    # lower per-iteration dispatch overhead than fori_loop (see _scan_loop).
    (_, _, min_cost_final), _ = jax.lax.scan(step_body, init_carry, xs=u_seq[1:num_steps])
