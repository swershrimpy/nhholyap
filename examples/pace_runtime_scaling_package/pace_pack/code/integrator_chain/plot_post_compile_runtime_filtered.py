...
# assigned in fixed order. Validated as a 2-slot categorical palette against
# the light surface: all six checks PASS (worst CVD dE 24.7 protan / 32.7
# tritan, normal-vision dE 33.6, both series >= 3:1 on surface).
...
def load_rows(csv_path: Path = _DATA_CSV):
    """Returns (Ns, run_time_ms, peak_mem_mb).

    peak_mem_mb is None if the CSV predates the peak_gpu_mem_bytes column, or
    a list with None at any order whose snapshot lacked device stats -- both
    are handled by the caller rather than crashing here.
    """
    ...
    data = sorted(
        (int(r["N"]), float(r["run_time_ms"]), _mem(r) if has_mem else None)
        for r in rows
    )
    Ns = [n for n, _, _ in data]
    run_ms = [v for _, v, _ in data]
    mem_mb = [m for _, _, m in data]
    if not has_mem or all(m is None for m in mem_mb):
        return Ns, run_ms, None
    return Ns, run_ms, mem_mb
...
def make_plot(Ns, vals, is_outlier, out_path: Path = _OUT_PDF,
    ...
    """Run time vs. N, optionally with peak GPU memory overlaid.

    twin_axis=True  -> memory on a right-hand y-axis of the same panel (the
                       requested form; see the module docstring's caveat about
                       the two scales being independent).
    twin_axis=False -> two side-by-side panels, no shared/twin scale.
    """
    ...
    if not have_mem:
        ...
        fig, (ax, ax_mem) = plt.subplots(1, 2, figsize=(11, 4.5))
        ax.plot(kept_N, kept_v, color=_COLOR, linewidth=1.6,
